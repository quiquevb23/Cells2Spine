...
model_dir = os.path.join(base_dir, "celltypist_models")
os.makedirs(model_dir, exist_ok=True)

output_base_dir = args.output_base_dir

def annotate_with_tabulae(adata, single_cell_ref_h5, model_dir, output_dir, base_dir):
    adata_ref = sc.read(single_cell_ref_h5)
    adata_ref_subs = adata_ref[adata_ref.obs['label'].isin(['uninjured', '7d', '14d'])].copy()
    ...
    model_path = os.path.join(model_dir, f"model_for_joined.pkl")
    ...
    plt.savefig(os.path.join(output_dir, f"celltypist_umap_joined.png"), bbox_inches='tight')
    plt.close()

    celltypist.dotplot(predictions, use_as_reference = 'leiden_0.4', use_as_prediction = 'majority_voting')
    plt.savefig(os.path.join(output_dir, f"celltypist_dotplot_joined.png"), bbox_inches='tight')
    plt.close()
    ...
    rank_genes_with_check(adata, 'predicted_labels', os.path.join(output_dir, f"rank_genes_groups_predicted_labels_joined.png"))

    # Perform DEA for leiden clusters
    rank_genes_with_check(adata, 'leiden_0.4', os.path.join(output_dir, f"rank_genes_groups_leiden_joined.png"))
    ...
    plt.savefig(os.path.join(output_dir, f"overlap_heatmap_joined.png"), bbox_inches='tight')
    plt.close()
    
    adata.write_h5ad(os.path.join(base_dir, f"joined_annotated.h5ad"))
    '''
...
# Loop over all subdirectories (there is only 1 now)
for file_name in os.listdir(base_dir):
    if file_name.startswith('adata_integrated.h5ad'):
        output_dir = os.path.join(output_base_dir, "Celltypist")
        os.makedirs(output_dir, exist_ok=True)

        adata = sc.read_h5ad(os.path.join(base_dir, file_name))
        annotate_with_tabulae(adata, single_cell_ref_h5, model_dir, output_dir, base_dir)
