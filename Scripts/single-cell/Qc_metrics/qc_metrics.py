...
for file_name in os.listdir(base_dir):
    ...
    for file in os.listdir(sample_path):
        if file.endswith('filtered_feature_bc_matrix.h5'):
            ...
            plt.savefig(os.path.join(output_dir, 'pct_counts_mt_displot.png'))
            ...
            plt.savefig(os.path.join(output_dir, 'scatter_centered_median.png'))
            plt.close()
            sc.pl.scatter(adata_filtered, x='total_counts', y='n_genes_by_counts', color='pct_counts_ribo', 
                          title='Filtered Scatter Plot (centered around median)', show=False)
            plt.savefig(os.path.join(output_dir, 'scatter_centered_median_ribo.png'))
            plt.close()
            # Distplot centered in median
            sns.displot(adata_filtered.obs['total_counts'], bins=100, kde=False)
            plt.savefig(os.path.join(output_dir, 'total_counts_centered_median_displot.png'))
            plt.close()
            sns.displot(adata_filtered.obs['pct_counts_mt'], bins=100, kde=False)
            plt.savefig(os.path.join(output_dir, 'pct_counts_mt_centered_median_displot.png'))
            plt.close()
            ...
            # We will define cells as being outliers if they have MAD >3 for pct_counts_mt, or for pct_counts_ribo
