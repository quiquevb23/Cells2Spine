...
def run_ingest(adata, single_cell_ref_h5, output_dir, base_dir):
    ...
    plt.savefig(os.path.join(output_dir, f"Ingest_UMAP_joined.png"), bbox_inches='tight')
    plt.close()
    ...
    plt.savefig(os.path.join(output_dir, f"Ingest_dotplot_joined.png"), bbox_inches='tight')
    plt.close()
    ...
    rank_genes_with_check(adata, 'cell_type', os.path.join(output_dir, f"Ingest_rank_genes_groups_predicted_annotations_joined.png"))
    ...
    plt.savefig(os.path.join(output_dir, f"Ingest_overlap_heatmap_joined.png"), bbox_inches='tight')
    plt.close()


    # Save the updated AnnData object
    adata.write_h5ad(os.path.join(base_dir, f"joined_annotated_ingest.h5ad"))
...
for file_name in os.listdir(base_dir):
    if file_name.startswith('adata_integrated.h5ad'):
        output_dir = os.path.join(output_base_dir, "Ingest")
        os.makedirs(output_dir, exist_ok=True)

        adata = sc.read_h5ad(os.path.join(base_dir, file_name))
        run_ingest(adata, single_cell_ref_h5, output_dir, base_dir)