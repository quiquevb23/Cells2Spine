...
####
    ...
    sc.tl.leiden(adata_noint, resolution=0.8, key_added='leiden_0.8')
    sc.tl.umap(adata_noint)
    sc.pl.umap(adata_noint, color=["sample", "condition", "leiden_0.4", "leiden_0.8"], wspace=0.5)
    ...
    for file in os.listdir(sample_path):
        if file.endswith('qc_metrics.h5ad'): # we take the raw data
            ...
            if file_name == "Spatial_1" or file_name == "Spatial_2":
                ...
                datas.append(adata_dict[sample_name]) #add to healthy adatas after SME normalization (approaches 2,3)
                #datas.append(adata) #append without processing raw (approach 1)
            else:
                adata.obs["condition"] = "injured"
                adata_dict[sample_name] = stlearn_clustering(adata, sample_name, output_dir) # perform clustering and return adata
                adata_dict[sample_name].write(os.path.join(sample_path, f"adata_stlearn_domains_{sample_name}.h5ad"))
                datas.append(adata_dict[sample_name])
                #datas.append(adata) #append without processing raw (approach 1) 
...
'''
adata_all = sc.concat(datas, join='inner', index_unique = "_")

process_combined(adata_all)
