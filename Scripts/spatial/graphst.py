...
for file_name in os.listdir(base_dir):
    ...
    for file in os.listdir(sample_path):
        if file.endswith('sctransformed.h5ad'):
...
#            sc.pp.normalize_total(adata) #maybe this step should not be present
...
#            sc.pp.scale(adata_hvg)
            ...
            #We give the normalized counts by SCTransform to GraphST
#            adata.X = adata.layers['sctransform']
            adata_hvg = adata.copy() # NO HVG selection
            adata_hvg.X = adata.layers["sctransform"] #use this layer from SCTransform 
            ...
            radii = [1, 6, 12, 16, 20, 30, 50]

            tool = 'mclust' # mclust, leiden, and louvain

            output_dir = os.path.join(output_dir, "graphst_domains_noHVG_sctransform")
            os.makedirs(output_dir, exist_ok=True)
            ...
            for radius in radii:
                if tool == 'mclust':
                   clustering(adata_hvg, n_clusters, radius=radius, method=tool, refinement=True) # For DLPFC dataset, we use optional refinement step.
            ...
            # plotting spatial clustering result
                sc.pl.spatial(adata_hvg,
                  ...
                  color=["domain"],
                  show=False)
                plt.savefig(os.path.join(output_dir, f'graphst_domains_{radius}.png'), bbox_inches='tight')
                plt.close()
